...
# ==============================================================================
CONFIG = {
    'WS_URL': "wss://chatp.net:5333/server",  # Server URL
    'BOT_NAME': "delvina",                  # Apne Bot ka naam
    'BOT_PASS': "p99665",                   # Bot ka password
    'ROOM': "قهوهـ_🇸🇦_السعـوديـه",                    # Target Room name (sahi spelling likhna)
    'DB_FILE': "limbo_game.db"                # Local file name (ignore for Render)
}

# GAME ASSETS (Images/GIFs)
ASSETS = {
    'start': "https://i.imgur.com/8QzXy9M.gif",    # Game Start
...
# ==============================================================================
# 2. SMART DATABASE (Render & PC Dono pe chalega)
# ==============================================================================
def get_db_connection():
    database_url = os.environ.get('DATABASE_URL') # Render se URL uthayega
    if database_url:
        try:
...
def init_db():
    ...
    ph = "SERIAL" if db_type == "pg" else "INTEGER" # Postgres vs Sqlite syntax
...
class LimboSession:
    def __init__(self, user, bet_amount=0):
        ...
        self.round = 1
        self.height = 0
        self.obstacle = None # 'low' or 'high'
    
    def next_round(self):
        # Logic: 
        # Height 20-59cm = Too Low = Must JUMP
        # Height 60-120cm = Too High = Must DUCK
        self.height = random.randint(20, 120)
        
        if self.height < 60:
            self.obstacle = 'low'
            return f"⚠️ BAR IS LOW ({self.height}cm)! SPIKES ON FLOOR!"
        else:
            self.obstacle = 'high'
            return f"⚠️ BAR IS HIGH ({self.height}cm)! LASER OVERHEAD!"
...
# ==============================================================================
# 5. FLASK LEADERBOARD (THE BEAUTIFUL PART)
# ==============================================================================
HTML_UI = """
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="header"><h1 class="title">TITAN LEADERBOARD</h1></div>
    <div class="lb-container" id="board"><div style="text-align:center; margin-top:50px; color:#00f3ff">INITIALIZING...</div></div>
    ...
    <script>
        ...
        async function update() {
            try {
                ...
                if(document.querySelector('.lb-container div').innerText === "INITIALIZING...") board.innerHTML = "";
